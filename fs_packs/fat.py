...
def _scan_fat_free(fh, bpb):
    ...
    if ftype == 32:
        ...
        c = 2
        while c < end_c:
            n = min(chunk_bytes // 4, end_c - c)
            buf = _read_at(fh, fat_start + c * 4, n * 4)
            if len(buf) < n * 4:
            ...
            for val in arr:
                ...
                c += 1
        if run_start is not None:
            runs.append((run_start, end_c - run_start))
        return runs, free
...
def collect_exfat(dev):
    ...
    flags = ((bm[i >> 3] >> (i & 7)) & 1 for i in range(cluster_count))
    runs, free = _runs_from_flags((not f for f in flags), cluster_count)
